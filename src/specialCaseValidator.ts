...
function validateFilipinoVat(vat: string): boolean {
  ...
  for (let i = 1; i < 7; i++) {
    if (
      parseInt(sequentialDigits[i]) !==
      parseInt(sequentialDigits[i - 1]) + 1
    ) {
      return false;
    }
  }
...
function validateColombianVat(vat: string): boolean {
  ...
  const calculatedCheckDigit =
    remainder == 0 || remainder == 1 ? remainder : 11 - remainder;
  return parseInt(checkDigit) === calculatedCheckDigit;
}
...
export const countrySpecialCaseMap: Map<Country, (vat: string) => boolean> =
  new Map([
